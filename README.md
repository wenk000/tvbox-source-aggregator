...
> **Fork 自** [tengxiaobao/tvbox-source-aggregator](https://gitee.com/tengxiaobao/tvbox-source-aggregator)，在原项目基础上增加了**聚合日志**功能。
...
- **站点去重** — 不同源中的相同站点只保留一份；相似名称站点合并，**只保留响应速度最快的线路**
- **自定义排序** — 按关键词规则自定义站点排列顺序（如体育类排最前、4K 类排第二）
- **Spider JAR 智能分配** — 自动处理 type:3 站点的 JAR 依赖冲突
- **测速筛选**（可选） — 自动过滤不可达或高延迟的源
- **内网 JSON 下载** — 提供 `/index.json` 和 `/live.json` 直接下载链接，解决影视仓等客户端无法配置内网地址的问题
- **背景自定义** — 管理页设置的背景主题在所有页面（管理、状态、配置编辑）统一生效
- **管理后台** — 网页端添加/删除源，触发刷新
- **定时更新** — 每天自动重新聚合，客户端无感知
- **容错设计** — 聚合失败时保留上次有效缓存
- **聚合日志** — 记录每次聚合的详细变更：新增/移除站点、失败的源、耗时统计
...
```bash
git clone https://gitee.com/yygitee118/tvbox-source-aggregator.git
...
ADMIN_TOKEN=你的管理密码
PORT=5678
BASE_URL=http://你的IP:5678
```

> `BASE_URL` 在 Docker 部署时**必填**，用于 JAR 代理地址改写。
...
影视仓等部分客户端不支持只填根路径，需要填写完整的 JSON 下载地址：

```
http://你的地址:5678/index.json
```

> 管理后台状态页和设置页均可一键复制此地址。

### 局域网内使用

部署完成后，**同一局域网内的所有设备**都可以直接使用你的聚合地址，无需额外配置。

**前提条件：**
- 运行本服务的设备与 TVBox 设备（手机、电视盒子等）连接**同一个路由器/WiFi**
- 服务器的防火墙允许 `5678` 端口访问（Docker 部署通常已自动开放）

**步骤：**

1. 查看服务器的局域网 IP 地址（例如 `192.168.2.16`）
...
2. 在 TVBox 中配置接口地址：
   ```
   http://192.168.2.16:5678/
   ```
   把 `192.168.2.16` 替换成你实际的服务器 IP。

3. TVBox 会自动从该地址拉取聚合后的配置。

> **提示**：如果 TVBox 无法连接，请检查路由器是否开启了「AP 隔离」（客户端隔离）功能，关闭它即可让局域网设备互通。

### 聚合日志（Logs 标签页）

管理后台新增 **Logs** 标签页，每次聚合后自动记录：
...
管理后台 → Settings → 「融合源分组排序」区域：

1. 勾选「启用分组排序」
2. 点击「+ 添加规则」，填写规则名称和关键词
   - 例如：名称填 `体育`，关键词填 `CCTV5,体育,NBA,足球,篮球`
   - 例如：名称填 `4K`，关键词填 `4K,4k,UHD,超清`
3. 用 ▲▼ 按钮调整规则优先级（排在前的规则优先匹配）
4. 选择「未匹配的位置」（排在最后 / 排在最前）
5. 点击保存，**下次聚合时生效**

站点名称包含任意关键词即归入该规则分组，按规则顺序排列。
...
| `/index.json` | GET | 配置 JSON 直接下载（影视仓等不支持路径配置的客户端用此地址） |
| `/live-config` | GET | 直播配置页面 |
| `/live.json` | GET | 直播 JSON 直接下载 |
| `/status` | GET | 监控仪表盘页面 |
| `/status-data` | GET | 状态数据 JSON |
| `/admin` | GET | 管理后台（密码保护） |
| `/admin/sources` | GET/POST/DELETE | 源管理 API（密码保护） |
| `/admin/agg-logs` | GET/DELETE | 聚合日志 API（密码保护） |
...
│   │   ├── admin.ts       # 管理后台页面（含聚合日志）
│   │   ├── dashboard.ts   # 监控仪表盘页面
│   │   ├── types.ts       # TypeScript 类型（含聚合日志类型）
│   │   └── config.ts      # 配置常量
│   ├── storage/           # 存储抽象层
